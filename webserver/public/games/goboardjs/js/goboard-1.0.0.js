...
;(function () {
 ...
 // number of rows and columns
  const NUM_FIELDS = 19;
  console.log("NUM_FIELDS= ", NUM_FIELDS);
	       
//  var COLUMNS = 'abcdefgh'.split('')
  var COLUMNS = 'abcdefghijklmnopqrstuvwxyz'.split('').slice(0, NUM_FIELDS);
  var DEFAULT_DRAG_THROTTLE_RATE = 20
  var ELLIPSIS = '…'
  var MINIMUM_JQUERY_VERSION = '1.8.3'
  var RUN_ASSERTS = false
	       //var START_FEN = '8/8/8/8/8/8/8/8' //'rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR
  // start with an empy board	      
  var START_FEN = (NUM_FIELDS+"/").repeat(NUM_FIELDS-1)+NUM_FIELDS;
  ...
  var CSS = {}
  CSS['alpha'] = 'alpha-go-d2270'
  CSS['black'] = 'black-go-3c85d'
  CSS['board'] = 'board-go-b72b1'
  CSS['chessboard'] = 'chessboard-go-63f37'
  CSS['clearfix'] = 'clearfix-go-7da63'
  CSS['highlight1'] = 'highlight1-go-32417'
  CSS['highlight2'] = 'highlight2-go-9c5d2'
  CSS['notation'] = 'notation-go-322f9'
  CSS['numeric'] = 'numeric-go-fc462'
  CSS['piece'] = 'piece-go-417db'
  CSS['row'] = 'row-go-5277c'
  CSS['sparePieces'] = 'spare-pieces-go-7492f'
  CSS['sparePiecesBottom'] = 'spare-pieces-bottom-go-ae20f'
  CSS['sparePiecesTop'] = 'spare-pieces-top-go-4028b'
  CSS['square'] = 'square-go-55d63'
  CSS['square-dot'] = 'square-dot-go-g9jg4'
  CSS['white'] = 'white-go-1e1d7'
  ...
  function validSquare (square) {
    return isString(square) && square.search(/^[a-z][0-9]+$/) !== -1
  }
  ...
  function validFen (fen) {
      if (!isString(fen))  {
	  console.log("FEN is not a string ", typeof fen, fen);
	  return false
      }
    ...
    // FEN should be NUM_FIELDS sections separated by slashes
    var chunks = fen.split('/')
      if (chunks.length !== NUM_FIELDS) {
	  console.log("isValidFen : wrong number of chunks", chunks.length, " NUM_FIELDS= ", NUM_FIELDS);
	  return false
      }

    // check each section
    for (var i = 0; i < NUM_FIELDS; i++) {
      var row = chunks[i].split("_");
      if (row.length !== NUM_FIELDS ||
          chunks[i].search(/[^kqrnbpKQRNBP1_]/) !== -1) {
        return false
      }
    }
  ...
  if (RUN_ASSERTS) {
   ...
   function validPositionObject (pos) {
    console.log("position object = ", pos);
	  ...
	  console.log("validPositionObject is not plain object");
	  return false
      }
    for (var i in pos) {
	...
	if (!validSquare(i) || !validPieceCode(pos[i])) {
	    console.log("validSquare(i) = ", validSquare(i), i);
	    console.log("validPieceCode(pos[i]) = ", validPieceCode(pos[i]), pos[i]);
	  console.log("validPositionObject: invalid square or piece code");
          return false
      }
    }
  ...
  function fenToObj (fen) {
   ...
   var currentRow = NUM_FIELDS
    for (var i = 0; i < NUM_FIELDS; i++) {
      var row = rows[i].split("_")
	var colIdx = 0
  ...
  function objToFen (obj) {
	  ...
	  console.log("objToFen: invalid position object ", obj);
	  return false
      }
    var fen = ''
    ...
    for (var i = 0; i < NUM_FIELDS; i++) {
	  ...
	  row.push(pieceCodeToFen(obj[square]));
        } else {
            // empty space
	  row.push(1);
        }
...
*/
  function squeezeFenEmptySquares (fen) {
      for (var i=NUM_FIELDS; i>1; i--) {
	  var expanded = Array(i).fill(1).join("_");
	  var reg = new RegExp(expanded, "g");
	  fen = fen.replace(reg, i);
      }
      return fen;
  }
  ...
  function expandConfig (config) {
    ...
    if (!config.hasOwnProperty('pieceTheme') ||
      ...
      config.pieceTheme = './goboardjs/img/gopieces/wikipedia/{piece}.png'
    }
  ...
  function constructor (containerElOrString, config) {
	...
	console.log("setInitialState config.position = ", config.position, " currentPosition = ", currentPosition);
    }
    ...
    function calculateSquareSize () {
      ...
      while (boardWidth % NUM_FIELDS !== 0 && boardWidth > 0) {
        boardWidth = boardWidth - 1
      }

      return boardWidth / NUM_FIELDS
    }
    ...
    function buildBoardHTML (orientation) {
      ...
      for (var i = 0; i < NUM_FIELDS; i++) {
		...
		if ((j === 3) || (j === NUM_FIELDS-3-1)) {
		    squareType = 'square-dot';
		}
	    } else if (i === NUM_FIELDS-3-1) {
		if ((j === 3) || (j === NUM_FIELDS-3-1)) {
		    squareType = 'square-dot';
		}
	    }
          ...
          if (config.showNotation) {
            ...
            if ((orientation === 'white' && row === 1) ||
                (orientation === 'black' && row === NUM_FIELDS)) {
	...
	var pieces = Array(19).fill('wP');
      if (color === 'black') {
          pieces = Array(19).fill('bP');
      }
    ...
    widget.resize = function () {
      ...
      // set board width
      $board.css('width', squareSize * NUM_FIELDS + 'px')
  ...
  window['Goboard'] = constructor

  // expose util functions
  window['Goboard']['fenToObj'] = fenToObj
  window['Goboard']['objToFen'] = objToFen
